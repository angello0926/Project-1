...
function dropping (){
  var outoffield = false; //row doesn't >21
     ...
     for (var i=0;i<tetris.currentCoor.length;i++){
        if(tetris.currentCoor[i].row>21||checkingcollision(tetris.currentCoor)===1){
          outoffield =true;
          if (outoffield){
             tetris.origin.row--; //change origin
             tetris.currentCoor=shape(tetris.currentshape,tetris.origin); //update current coor
           //store the shape in the playfield
    ...
    fillcells(tetris.currentCoor,'#B0DACC');
...
function moveshape(direction){
     ...
     for(var i=0;i<tetris.currentCoor.length;i++){
        if (direction==='right'){
          ...
          if(tetris.currentCoor[i].col>9){  //check if the col no. is more than 9, then out of fields.
            outoffield=true;
          }

        }else if (direction==='left'){
          tetris.currentCoor[i].col--;
           if(tetris.currentCoor[i].col<0){  //check if the col no. is < 0, then out of fields.
            outoffield=true;
